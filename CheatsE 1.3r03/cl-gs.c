...
int DrawImage(u32 addr, int x, int y, int z, int w, int h, int fade)
{
	...
	{
		DrawPhotoSpot(addr, 0, 0, w, h, x, y, z, w, h, 0, 0, 0, fade, 0);
	}
...
int LoadTexture(int x, int y, int w, int h, int z, u32 addr, PACKET pck, int fade)
{
	...
	if (pck.data == NULL) { return -2; }

	for (y2 = 0; y2 < h; y2++)
	{
		for (x2 = 0; x2 < w; x2++)
		{
			if ( (pck.count + (sizeof(u64) * 10)) > pck.size ) { return -3; }
			...
			if (b == 0x00) {
				...
				*(u64*)(&pck.data[cnt]) = GIF_REG_RGBAQ;
				cnt += sizeof(u64);
				*(u64*)(&pck.data[cnt]) = GIF_SET_XYZ( (Left+ x2) << 4, (Top + y2) << 4, z);
				cnt += sizeof(u64);
				*(u64*)(&pck.data[cnt]) = GIF_REG_XYZ2;
				cnt += sizeof(u64);
				*(u64*)(&pck.data[cnt]) = GIF_SET_XYZ( ((Left + x2) + 1) << 4, ((Top + y2) + 1) << 4, z);
				cnt += sizeof(u64);
				*(u64*)(&pck.data[cnt]) = GIF_REG_XYZ2;
				cnt += sizeof(u64);
			skip:
...
void DrawPhotoSpot(u32 ImgAddr, int SpotX, int SpotY, int SpotW, int SpotH, int ImgX, int ImgY, int ImgZ, int ImgW, int ImgH, int Nr, int Ng, int Nb, int Fade, int RGBOrder)
{
